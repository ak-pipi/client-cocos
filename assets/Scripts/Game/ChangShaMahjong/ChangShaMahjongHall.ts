...
@ccclass('ChangShaMahjongHall')
export class ChangShaMahjongHall extends Component {
    ...
    public onDistrictClicked(event: Event, customEventData: any | null) {
        ...
        if (name === "Friend") {
            ...
            this.enterDistrict(DistrictId.ChangShaBeginner);
        }
        else if (name === "Moderate") {
            this.enterDistrict(DistrictId.ChangShaModerate);
        }
        else if (name === "Advanced") {
            this.enterDistrict(DistrictId.ChangShaAdvanced);
        }
        else if (name === "Master") {
            this.enterDistrict(DistrictId.ChangShaMaster);
        }
    }
    ...
    private createRoom(level: number) {
        ...
        console.log(text);
        let msg = {
            gameType: GameType.ChangShaMahjong,
            base64: text
        };
        GameManager.Instance.authPost("/player/game/create", msg).then((dto) => {
            ...
            GameManager.Instance.enterVenue(dto.wsAddress, dto.venueId, GameType.ChangShaMahjong, () => { this.onEnterVenue(); });
        }).catch((err) => {
    ...
    private onEnterVenue() {
        ResourceLoader.Instance.loadAsset("ChangShaMahjongRoomMain", "Room", Prefab, (prefab: Prefab) => {
            if (!prefab) {
    ...
    public onNumClicked(event: Event, customEventData: any | null) {
        ...
        if (this.count == 6) {
            ...
            let gameType: number = GameType.ChangShaMahjong;
            let data = {
                number: number,
                gameType: gameType
            };

...
@ccclass('TaoJiangMahjongHall')
export class TaoJiangMahjongHall extends Component {
    @property({ type: Node })
    ...
    public onDistrictClicked(event: Event, customEventData: any | null) {
        ...
        if (name === "Friend") {
            ...
            this.enterDistrict(DistrictId.TaoJiangBeginner);
        }
        else if (name === "Moderate") {
            this.enterDistrict(DistrictId.TaoJiangModerate);
        }
        else if (name === "Advanced") {
            this.enterDistrict(DistrictId.TaoJiangAdvanced);
        }
        else if (name === "Master") {
            this.enterDistrict(DistrictId.TaoJiangMaster);
        }
    }
    ...
    private createRoom(level: number) {
        ...
        console.log(text);
        let msg = {
            gameType: GameType.TaoJiangMahjong,
            base64: text
        };
        GameManager.Instance.authPost("/player/game/create", msg).then((dto) => {
            ...
            GameManager.Instance.enterVenue(dto.wsAddress, dto.venueId, GameType.TaoJiangMahjong, () => { this.onEnterVenue(); });
        }).catch((err) => {
    ...
    private onEnterVenue() {
        ResourceLoader.Instance.loadAsset("TaoJiangMahjongRoomMain", "Room", Prefab, (prefab: Prefab) => {
            if (!prefab) {
    ...
    public onNumClicked(event: Event, customEventData: any | null) {
        ...
        if (this.count == 6) {
            ...
            let gameType: number = GameType.TaoJiangMahjong;
            let data = {
                number: number,
                gameType: gameType
            };

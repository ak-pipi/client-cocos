...
@ccclass('PaoDeKuaiHall')
export class PaoDeKuaiHall extends Component {
    @property({ type: Node })
    ...
    public onDistrictClicked(event: Event, customEventData: any | null) {
        ...
        if (name === "Friend") {
            ...
            this.enterDistrict(DistrictId.PaoDeKuaiBeginner);
        }
        else if (name === "Moderate") {
            this.enterDistrict(DistrictId.PaoDeKuaiModerate);
        }
        else if (name === "Advanced") {
            this.enterDistrict(DistrictId.PaoDeKuaiAdvanced);
        }
        else if (name === "Master") {
            this.enterDistrict(DistrictId.PaoDeKuaiMaster);
        }
    }
    ...
    private createRoom(level: number) {
        ...
        GameManager.Instance.authPost("/player/game/create", msg).then((dto) => {
            ...
            GameManager.Instance.enterVenue(dto.wsAddress, dto.venueId, GameType.PaoDeKuai, () => { this.onEnterVenue(); });
        }).catch((err) => {
    ...
    private onEnterVenue() {
        ResourceLoader.Instance.loadAsset("PaoDeKuaiRoomMain", "Room", Prefab, (prefab: Prefab) => {
            if (!prefab) {
    ...
    public onNumClicked(event: Event, customEventData: any | null) {
        ...
        if (this.count == 6) {
            ...
            let gameType: number = GameType.PaoDeKuai;
            let data = {
                number: number,
                gameType: gameType
            };

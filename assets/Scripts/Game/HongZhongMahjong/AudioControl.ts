...
export class AudioControl extends Component {
    ...
    start() {
        ...
        ResourceLoader.Instance.loadAsset("HongZhongMahjongAudio", "bg", AudioClip, (clip: AudioClip) => {
    ...
    public playStart() {
        ...
        ResourceLoader.Instance.loadAsset("HongZhongMahjongAudio", "gamestart", AudioClip, (clip: AudioClip) => {
            if (clip) {
    ...
    // 播放发牌声音
    private playDeal() {
        ResourceLoader.Instance.loadAsset("HongZhongMahjongAudio", "dealcard", AudioClip, (clip: AudioClip) => {
            if (clip) {
                this.loopSource.stop();
    ...
    // 播放摸牌声音
    public playFetch(male: boolean, clientSeat: number) {
        if (GameManager.Instance.SoundMute) return;
        if (!this.gameSources[clientSeat]) return;
        let clipName: string = male ? "Action/Male/fetch" : "Action/Female/fetch";
        ResourceLoader.Instance.loadAsset("HongZhongMahjongAudio", clipName, AudioClip, (clip: AudioClip) => {
            if (clip) {
    ...
    public playPlay(male: boolean, clientSeat: number) {
        if (GameManager.Instance.SoundMute) return;
        if (!this.gameSources[clientSeat]) return;
        let clipName: string = male ? "Action/Male/play" : "Action/Female/play";
        ResourceLoader.Instance.loadAsset("HongZhongMahjongAudio", clipName, AudioClip, (clip: AudioClip) => {
            if (clip) {
                this.gameSources[clientSeat].stop();
                this.gameSources[clientSeat].clip = clip;
    ...
    public playAlert() {
        ...
        ResourceLoader.Instance.loadAsset("HongZhongMahjongAudio", "warning", AudioClip, (clip: AudioClip) => {
            if (clip) {
                this.shareSource.stop();
                this.shareSource.clip = clip;
                this.shareSource.play();

...
@ccclass('HongZhongMahjongHall')
export class HongZhongMahjongHall extends Component {
    @property({ type: Node })
    ...
    public onDistrictClicked(event: Event, customEventData: any | null) {
        ...
        if (name === "Friend") {
            ...
            this.enterDistrict(DistrictId.HongZhongBeginner);
        }
        else if (name === "Moderate") {
            this.enterDistrict(DistrictId.HongZhongModerate);
        }
        else if (name === "Advanced") {
            this.enterDistrict(DistrictId.HongZhongAdvanced);
        }
        else if (name === "Master") {
            this.enterDistrict(DistrictId.HongZhongMaster);
        }
    }
    ...
    private createRoom(level: number) {
        ...
        console.log(text);
        let msg = {
            gameType: GameType.HongZhongMahjong,
            base64: text
        };
        GameManager.Instance.authPost("/player/game/create", msg).then((dto) => {
            ...
            GameManager.Instance.enterVenue(dto.wsAddress, dto.venueId, GameType.HongZhongMahjong, () => { this.onEnterVenue(); });
        }).catch((err) => {
    ...
    private onEnterVenue() {
        ResourceLoader.Instance.loadAsset("HongZhongMahjongRoomMain", "Room", Prefab, (prefab: Prefab) => {
            if (!prefab) {
    ...
    public onNumClicked(event: Event, customEventData: any | null) {
        ...
        if (this.count == 6) {
            ...
            let gameType: number = GameType.HongZhongMahjong;
            let data = {
                number: number,
                gameType: gameType
            };

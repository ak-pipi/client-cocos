...
@ccclass('YuanJiangQianFenHall')
export class YuanJiangQianFenHall extends Component {
    @property({ type: Node })
    ...
    public onDistrictClicked(event: Event, customEventData: any | null) {
        ...
        if (name === "Friend") {
            ...
            this.enterDistrict(DistrictId.YuanJiangQianFenBeginner);
        }
        else if (name === "Moderate") {
            this.enterDistrict(DistrictId.YuanJiangQianFenModerate);
        }
        else if (name === "Advanced") {
            this.enterDistrict(DistrictId.YuanJiangQianFenAdvanced);
        }
        else if (name === "Master") {
            this.enterDistrict(DistrictId.YuanJiangQianFenMaster);
        }
    }
    ...
    private createRoom(level: number) {
        ...
        console.log(text);
        let msg = {
            gameType: GameType.YuanJiangQianFen,
            base64: text
        };
        GameManager.Instance.authPost("/player/game/create", msg).then((dto) => {
            ...
            GameManager.Instance.enterVenue(dto.wsAddress, dto.venueId, GameType.YuanJiangQianFen, () => { this.onEnterVenue(); });
        }).catch((err) => {
    ...
    private onEnterVenue() {
        ResourceLoader.Instance.loadAsset("YuanJiangQianFenRoomMain", "Room", Prefab, (prefab: Prefab) => {
            if (!prefab) {
    ...
    public onNumClicked(event: Event, customEventData: any | null) {
        ...
        if (this.count == 6) {
            ...
            let gameType: number = GameType.YuanJiangQianFen;
            let data = {
                number: number,
                gameType: gameType
            };

...
@ccclass('WaiHuZiHall')
export class WaiHuZiHall extends Component {
    @property({ type: Node })
    ...
    public onDistrictClicked(event: Event, customEventData: any | null) {
        ...
        if (name === "Friend") {
            ...
            this.enterDistrict(DistrictId.YiYangWaiHuZiBeginner);
        }
        else if (name === "Moderate") {
            this.enterDistrict(DistrictId.YiYangWaiHuZiModerate);
        }
        else if (name === "Advanced") {
            this.enterDistrict(DistrictId.YiYangWaiHuZiAdvanced);
        }
        else if (name === "Master") {
            this.enterDistrict(DistrictId.YiYangWaiHuZiMaster);
        }
    }
    ...
    private createRoom(level: number) {
        ...
        let msg = {
            gameType: GameType.YiYangWaiHuZi,
            base64: text
        };
        GameManager.Instance.authPost("/player/game/create", msg).then((dto) => {
            ...
            GameManager.Instance.enterVenue(dto.wsAddress, dto.venueId, GameType.YiYangWaiHuZi, () => { this.onEnterVenue(); });
        }).catch((err) => {
    ...
    private onEnterVenue() {
        ResourceLoader.Instance.loadAsset("WaiHuZiRoomMain", "Room", Prefab, (prefab: Prefab) => {
            if (!prefab) {
    ...
    public onNumClicked(event: Event, customEventData: any | null) {
        ...
        if (this.count == 6) {
            ...
            let gameType: number = GameType.YiYangWaiHuZi;
            let data = {
                number: number,
                gameType: gameType
            };
